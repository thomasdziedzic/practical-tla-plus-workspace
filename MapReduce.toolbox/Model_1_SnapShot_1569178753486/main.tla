...
CONSTANTS ItemRange, ItemCount, Workers, Reducer, NULL
...
variables
    ...
    queue = [w \in Workers |-> <<>>],
    status = [w \in Workers |-> "active"]; \* only the reducer should touch this
...
begin
    ...
    ReduceResult:
        while ActiveWorkers /= {} do
            ...
            or
                \* Reassign
                with from_worker \in ActiveWorkers \ FairWorkers, to_worker \in HealthyWorkers \ {from_worker} do
...
fair process worker \in UnfairWorkers
begin RegularWorker:
    call work();
end process;
...
ReduceResult == /\ pc[Reducer] = "ReduceResult"
                /\ IF ActiveWorkers /= {}
                      THEN /\ \/ /\ \E w \in {w \in ActiveWorkers: result[w].count = Len(assignments[w])}:
                           ...
                           /\ pc' = [pc EXCEPT ![Reducer] = "ReduceResult"]
                      ELSE /\ Assert(SumSeq(final) = SumSeq(input), 
...
Spec == /\ Init /\ [][Next]_vars
        ...
        /\ \A self \in UnfairWorkers : WF_vars(worker(self)) /\ WF_vars(work(self))
...
\* Last modified Sun Sep 22 13:59:04 CDT 2019 by tom
